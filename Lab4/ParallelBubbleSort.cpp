...
// Parallel bubble sort algorithm (Odd-Even Transposition Sort)
void ParallelBubble(double *pProcData, int BlockSize) {
    ...
    for (int i = 0; i < ProcNum; i++) {
        if ((i % 2) == 1) {
            // Odd phase: odd ranks exchange with right neighbors
            if ((ProcRank % 2) == 1) {
                Offset = 1;
            ...
            // Even phase: even ranks exchange with right neighbors
            if ((ProcRank % 2) == 1) {
                Offset = -1;
                SplitMode = KeepSecondHalf;
            } else {
